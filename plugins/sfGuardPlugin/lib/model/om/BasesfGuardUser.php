...
abstract class BasesfGuardUser extends BaseObject  implements Persistent {
	...
	/**
	 * @var        array Wines[] Collection to store aggregation of Wines objects.
	 */
	protected $collWiness;

	/**
	 * @var        Criteria The criteria used to select the current contents of collWiness.
	 */
	private $lastWinesCriteria = null;
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		if ($deep) {  // also de-associate any related objects?
			...
			$this->collWiness = null;
			$this->lastWinesCriteria = null;
	...
	protected function doSave(PropelPDO $con)
	{
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->collWiness !== null) {
				foreach ($this->collWiness as $referrerFK) {
	...
	public function copyInto($copyObj, $deepCopy = false)
	{
		...
		if ($deepCopy) {
			...
			foreach ($this->getWiness() as $relObj) {
				if ($relObj !== $this) {  // ensure that we don't try to copy a reference to ourselves
					$copyObj->addWines($relObj->copy($deepCopy));
				}
			}
	...
	/**
	 * Clears out the collWiness collection (array).
	 *
	 ...
	 * @see        addWiness()
	 */
	public function clearWiness()
	{
		$this->collWiness = null; // important to set this to NULL since that means it is uninitialized
	}

	/**
	 * Initializes the collWiness collection (array).
	 *
	 * By default this just sets the collWiness collection to an empty array (like clearcollWiness());
	 * however, you may wish to override this method in your stub class to provide setting appropriate
	 * to your application -- for example, setting the initial array to the values stored in database.
	 *
	 * @return     void
	 */
	public function initWiness()
	{
		$this->collWiness = array();
	}

	/**
	 * Gets an array of Wines objects which contain a foreign key that references this object.
	 *
	 * If this collection has already been initialized with an identical Criteria, it returns the collection.
	 * Otherwise if this sfGuardUser has previously been saved, it will retrieve
	 * related Winess from storage. If this sfGuardUser is new, it will return
	 ...
	 * @throws     PropelException
	 */
	public function getWiness($criteria = null, PropelPDO $con = null)
	{
		if ($criteria === null) {
			$criteria = new Criteria(sfGuardUserPeer::DATABASE_NAME);
		}
		...
		if ($this->collWiness === null) {
			if ($this->isNew()) {
			   $this->collWiness = array();
				...
				$criteria->add(WinesPeer::USER_ID, $this->id);

				WinesPeer::addSelectColumns($criteria);
				$this->collWiness = WinesPeer::doSelect($criteria, $con);
			}
		} else {
			...
			if (!$this->isNew()) {
				...
				WinesPeer::addSelectColumns($criteria);
				if (!isset($this->lastWinesCriteria) || !$this->lastWinesCriteria->equals($criteria)) {
					$this->collWiness = WinesPeer::doSelect($criteria, $con);
				}
			}
		}
		$this->lastWinesCriteria = $criteria;
		return $this->collWiness;
	}

	/**
	 * Returns the number of related Wines objects.
	 *
	 * @param      Criteria $criteria
	 * @param      boolean $distinct
	 * @param      PropelPDO $con
	 * @return     int Count of related Wines objects.
	 * @throws     PropelException
	 */
	public function countWiness(Criteria $criteria = null, $distinct = false, PropelPDO $con = null)
	{
		if ($criteria === null) {
			$criteria = new Criteria(sfGuardUserPeer::DATABASE_NAME);
		} else {
		...
		if ($this->collWiness === null) {
			if ($this->isNew()) {
				...
				$count = WinesPeer::doCount($criteria, false, $con);
			}
		} else {
			// criteria has no effect for a new object
			if (!$this->isNew()) {
	...
	/**
	 * Method called to associate a Wines object to this object
	 * through the Wines foreign key attribute.
	 *
	 * @param      Wines $l Wines
	 * @return     void
	 * @throws     PropelException
	 */
	public function addWines(Wines $l)
	{
		if ($this->collWiness === null) {
			$this->initWiness();
		}
		if (!in_array($l, $this->collWiness, true)) { // only add it if the **same** object is not already associated
			array_push($this->collWiness, $l);
	...
	public function clearAllReferences($deep = false)
	{
		if ($deep) {
			if ($this->collWiness) {
				foreach ((array) $this->collWiness as $o) {
